...
def initialize_parameter():
    ...
    parser.add_argument('--root_path', type=str, default='./data/rose/', help='数据文件的根路径（root path of the data file）')
    ...
    parser.add_argument('--batch_size', type=int, default=8, help='训练输入数据的批大小 batch size of train input data--------------------批次大小')
    ...
    parser.add_argument('--pred_len', type=int, default=8, help='预测序列长度（prediction sequence length）原始默认为24------------------预测序列长度pred_len')
    ...
    parser.add_argument('--sub_them', type=str, default='1变量一对一', help='单次运行的存储文件夹字后面的内容--------------------存储数据父文件夹名字')
    ...
    parser.add_argument('--data', type=str, required=False, default='rose', help='data them，取决了在data parse中寻找的是哪个数据文件的配置,很重要')
